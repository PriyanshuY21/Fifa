...
api_key = '0b765c35e0602f42e3274cf70cb1c2f6'
...
try:
    ...
    for team_link, club_name in team_links:
        ...
        try:
            ...
            # Find the table container for playing time stats
            playing_time_stats_container = team_soup.find('div', {'id': 'div_stats_playing_time_combined'})
            if playing_time_stats_container:
                player_stats_table = playing_time_stats_container.find('table')
    ...
    df.to_csv('all_teams_playing_time_stats_all_comps.csv', index=False)

    # Save DataFrame to JSON
    df.to_json('all_teams_playing_time_stats_all_comps.json', orient='records', lines=True)

    print("Data has been successfully saved to 'all_teams_playing_time_stats_all_comps.csv' and 'all_teams_playing_time_stats_all_comps.json'.")

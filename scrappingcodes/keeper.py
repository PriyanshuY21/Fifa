...
try:
    ...
    for team_link, club_name in team_links:
        ...
        keeper_stats_table = team_soup.find('div', {'id': 'div_stats_keeper_combined'})
        # adv_keeper_stats_table = team_soup.find('div', {'id': 'div_stats_keeper_adv_combined'})

        for table_container in [keeper_stats_table]:
            if table_container:
    ...
    df.to_csv('all_teams_keeper_stats_all_comps.csv', index=False)

    # Save DataFrame to JSON
    df.to_json('all_teams_keeper_stats_all_comps.json', orient='records', lines=True)

    print("Data has been successfully saved to 'all_teams_keeper_stats_all_comps.csv' and 'all_teams_keeper_stats_all_comps.json'.")

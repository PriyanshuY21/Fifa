...
try:
    ...
    for team_link, club_name in team_links:
        ...
        try:
            ...
            # Find the table container for passing types stats
            passing_types_container = team_soup.find('div', {'id': 'div_stats_passing_types_combined'})
            if passing_types_container:
                player_stats_table = passing_types_container.find('table')
    ...
    df.to_csv('all_teams_passing_types_stats_all_comps.csv', index=False)

    # Save DataFrame to JSON
    df.to_json('all_teams_passing_types_stats_all_comps.json', orient='records', lines=True)

    print("Data has been successfully saved to 'all_teams_passing_types_stats_all_comps.csv' and 'all_teams_passing_types_stats_all_comps.json'.")

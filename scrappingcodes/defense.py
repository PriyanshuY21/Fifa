...
try:
    ...
    for team_link, club_name in team_links:
        ...
        try:
            ...
            # Find the table container for defense stats
            defense_stats_container = team_soup.find('div', {'id': 'div_stats_defense_combined'})
            if defense_stats_container:
                player_stats_table = defense_stats_container.find('table')
    ...
    df.to_csv('all_teams_defense_stats_all_comps.csv', index=False)

    # Save DataFrame to JSON
    df.to_json('all_teams_defense_stats_all_comps.json', orient='records', lines=True)

    print("Data has been successfully saved to 'all_teams_defense_stats_all_comps.csv' and 'all_teams_defense_stats_all_comps.json'.")

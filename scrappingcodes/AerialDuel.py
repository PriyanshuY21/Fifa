...
try:
    ...
    for team_link, club_name in team_links:
        ...
        try:
            ...
            # Find the table container for misc stats
            misc_stats_container = team_soup.find('div', {'id': 'div_stats_misc_combined'})
            if misc_stats_container:
                player_stats_table = misc_stats_container.find('table')
    ...
    df.to_csv('all_teams_misc_stats_all_comps.csv', index=False)

    # Save DataFrame to JSON
    df.to_json('all_teams_misc_stats_all_comps.json', orient='records', lines=True)

    print("Data has been successfully saved to 'all_teams_misc_stats_all_comps.csv' and 'all_teams_misc_stats_all_comps.json'.")

...
try:
    ...
    for team_link, club_name in team_links:
        ...
        try:
            ...
            # Find the table containing the shooting stats
            shooting_stats_container = team_soup.find('div', {'id': 'div_stats_shooting_combined'})
            if shooting_stats_container:
                player_stats_table = shooting_stats_container.find('table')
    ...
    df.to_csv('all_teams_shooting_stats_all_comps.csv', index=False)

    # Save DataFrame to JSON
    df.to_json('all_teams_shooting_stats_all_comps.json', orient='records', lines=True)

    print("Data has been successfully saved to 'all_teams_shooting_stats_all_comps.csv' and 'all_teams_shooting_stats_all_comps.json'.")
